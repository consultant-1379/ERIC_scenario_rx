...
final class StackTraceFilter {
    ...
    static final String RX_SCENARIO_RUNNER = RxScenarioRunner.class.getName();
    static final String RX_API = RxApi.class.getName();
    ...
    static <T extends Throwable> T filterFrameworkStackTrace(T e) {
        ...
        e.setStackTrace(from(stackTrace)
                .limit(limit)
                .filter(NO_RX_JAVA)
    ...
    static <T extends Throwable> T filterTestwareStackTrace(T e) {
        ...
        e.setStackTrace(from(stackTrace)
                .limit(limit)
                .filter(NO_RX_JAVA)
                .filter(NO_REFLECTIONS)
                .filter(NO_RX_SCENARIO)
                .toArray(StackTraceElement.class));
        if (e.getCause() != null) {
