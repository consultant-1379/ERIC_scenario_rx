...
class TooltipGenerator {
    ...
    private void addAttributeRow(HtmlBuilder builder, String key, String value) {
        builder.html("<tr>")
                .html("<td>").strong(key).html("</td>")
                .html("<td>").text(value).html("</td>")
                .html("</tr>");
    }

    private void addDataRecords(ScenarioExecutionGraph.GraphNode node, HtmlBuilder builder) {
        if (node.hasDataRecord()) {
            Map<String, RxDataRecord> dataRecords = parser.parseDataRecords(node.getDataRecord());
            builder.html("<table><tr><th>Data Source</th><th>Data Record</th><th>Value</th></tr>");
            for (Map.Entry<String, RxDataRecord> entry : dataRecords.entrySet()) {
                String dataSourceName = entry.getKey();
                RxDataRecord dataRecord1 = entry.getValue();
                boolean first = true;
