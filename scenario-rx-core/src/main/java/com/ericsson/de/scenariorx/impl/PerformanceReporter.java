...
public class PerformanceReporter {
    ...
    String summary(String name, List<TestStepResult> testStepResults) {
        ArrayListMultimap<String, TestStepResult> resultsByStep = ArrayListMultimap.create();
        ...
        for (TestStepResult result : testStepResults) {
        ...
        builder.append(format("%nSamples count: %.0f, %.2f%% (%.0f) failures", successes,
                failures / (successes == 0 ? 1 : successes), failures));
        builder.append("\nStats by Test Step: ");
    ...
    private void appendTestStepResults(final ArrayListMultimap<String, TestStepResult> resultsByStep,
            final StringBuilder builder) {
        for (Collection<TestStepResult> stepResults : resultsByStep.asMap().values()) {
            long timeSum = 0;
            long min = Long.MAX_VALUE;
            long max = 0;

            for (TestStepResult result : stepResults) {
            ...
            builder.append(format("%n  Name: %s, Samples: %s, Min: %s, Max: %s, Avg: %s",
                    getLast(stepResults).name,
                    stepResults.size(),
                    toHumanReadableDuration(min),
                    toHumanReadableDuration(max),
                    toHumanReadableDuration(timeSum / stepResults.size())));
        }
    }
