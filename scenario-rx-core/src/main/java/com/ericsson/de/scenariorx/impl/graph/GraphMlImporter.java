...
public class GraphMlImporter {
    ...
    private static ScenarioExecutionGraph parseGraph(URL graphText) throws IOException {
        ...
        for (Edge edge : doc.graph.edges) {
            GraphNode sourceVertex = nodes.get(edge.source);
            GraphNode targetVertex = nodes.get(edge.target);

            Map<String, String> attrMap = parseAttrMap(edge.data, replacements);
            String edgeLabel = attrMap.get(LabeledEdge.EDGE_LABEL);

            if (edgeLabel != null) {
                result.addEdge(sourceVertex, targetVertex, new LabeledEdge(edgeLabel));
            } else {
    ...
    private static Map<String, String> parseAttrMap(List<Map> data,
