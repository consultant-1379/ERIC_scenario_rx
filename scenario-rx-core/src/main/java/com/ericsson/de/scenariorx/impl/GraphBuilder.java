...
import com.ericsson.de.scenariorx.impl.graph.GraphNodeFactory;
...
class GraphBuilder {
    ...
    static ScenarioExecutionGraph build(List<Internals.FlowExecutionResult> results) {
        checkArgument(!results.isEmpty(), "No Flow Execution Results supplied");
    ...
    private static List<ScenarioExecutionGraph.GraphNode> addExpandedFork(ScenarioExecutionGraph graph,
                                                                          ScenarioExecutionGraph.GraphNode forkNode,
    ...
    private static ScenarioExecutionGraph.GraphNode addExecution(ScenarioExecutionGraph graph,
                                                                 ScenarioExecutionGraph.GraphNode firstNode, Internals.Exec execution) {
    ...
    private static ScenarioExecutionGraph.GraphNode addJoinedFork(ScenarioExecutionGraph graph,
                                                                  ScenarioExecutionGraph.GraphNode forkNode, Internals.FlowExecutionResult result) {
    ...
    private static ScenarioExecutionGraph.GraphNode createForkNode(Internals.Exec execution,
                                                                   Internals.FlowExecutionResult result) {
        return GraphNodeFactory.createSubFlowNode(
                result.id,
                result.name,
                execution.dataRecord.toString(),
                execution.vUser.getId(),
                "",
                result.error
        );
    }

    private static ScenarioExecutionGraph.TestStepNode createStepNode(Internals.Exec execution,
                                                                      Internals.TestStepResult result) {
        return GraphNodeFactory.createTestStepNode(
                result.id,
                result.name,
                result.startTime,
                result.endTime,
    ...
    private static ScenarioExecutionGraph.GraphNode createJoinNode(ScenarioExecutionGraph.GraphNode forkNode,
                                                                   Internals.FlowExecutionResult result) {
        return GraphNodeFactory.createFlowEndedNode(
                result.id,
                "join",
                forkNode.getDataRecord(),
                forkNode.getVUser(),
                "",
                forkNode.getException()
        );
    }

    private static void joinNodes(ScenarioExecutionGraph graph,
                                  List<ScenarioExecutionGraph.GraphNode> preJoinNodes,
    ...
    private static void addEdge(ScenarioExecutionGraph graph, Internals.Exec execution,
                                ScenarioExecutionGraph.GraphNode from, ScenarioExecutionGraph.GraphNode to) {
    ...
    private static boolean shouldLabelEdge(String vUserId, ScenarioExecutionGraph.GraphNode lastTestStep) {
        return !lastTestStep.getVUser().equals(vUserId)
                || lastTestStep instanceof ScenarioExecutionGraph.RootNode
