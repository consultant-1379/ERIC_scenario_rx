...
public class PerformanceTest {
    ...
    public void testRampup() throws Exception {
        ...
        PerformanceFlowBuilder flow = (PerformanceFlowBuilder) new PerformanceFlowBuilder("test")
                .addTestStep(new ScenarioTest.InlineInvocation() {
                ...
                .withVUsers(vUsers)
                .withDataSources(fromIterable("name", asList(1, 2, 3, 4, 5, 6, 7, 8, 9)).shared());

        flow.withRampUp(
                RxRampUp.during(rampUpTime, SECONDS)
        );

        timer.start();
        ScenarioRunner build = runner().build();
        build.runPerformance(flow);

        assertThat(stack).containsExactly(
                "thread-1 dr-1", "thread-1 dr-4", "thread-1 dr-5",
                "thread-2 dr-2", "thread-1 dr-6", "thread-2 dr-7", "thread-1 dr-8", "thread-2 dr-9",
                "thread-3 dr-3"
        );
    ...
    public void testDuration() throws Exception {
        ...
        PerformanceFlowBuilder flow = (PerformanceFlowBuilder) new PerformanceFlowBuilder("test")
                ...
                .runWhile(during(duration, SECONDS))
                .withVUsers(vUsers)
                .withDataSources(fromIterable("name", asList(1, 2, 3)).shared());
